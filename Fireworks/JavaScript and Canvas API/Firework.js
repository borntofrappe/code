class Firework {
  constructor() {
    ...
    const vy =
      Math.random() * (fireworkVelocityMax - fireworkVelocityMin) +
      fireworkVelocityMin;
    const velocity = {
      x: 0,
      y: vy * -1,
  ...
  update() {
    ...
    if (!this.hasExploded) {
      if (
        ...
        this.particle.velocity.y < fireworkVelocityThreshold
      ) {
        this.particle.r = Math.max(
          0,
          this.particle.r - FIREWORK_RADIUS_DECREASE
        );
      }
